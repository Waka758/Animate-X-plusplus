...
class WanModel(torch.nn.Module):
    ...
    def patchify(self, x: torch.Tensor): # torch.Size([1, 36, 21, 104, 60])
        # import pdb; pdb.set_trace()

        x = self.patch_embedding(x) # torch.Size([1, 5120, 21, 52, 30])
        grid_size = x.shape[2:]
    ...
    def forward(self,
        ...
        if pose_emb is not None: #   torch.Size([19, 15, 1, 1, 5120])
            # import pdb; pdb.set_trace()
            # pose_emb = pose_emb.reshape(x.shape[0], pose_emb.shape[0] // x.shape[0], -1, *pose_emb.shape[2:]) 
            pose_emb = pose_emb.reshape(x.shape[0], pose_emb.shape[0] // x.shape[0], 1, 1, pose_emb.shape[2]) 
        ...
        for layer_i, block in enumerate(self.blocks):
            ...
            if pose_emb is not None:
                if (layer_i <= len(self.blocks) // 2 and layer_i > 1): # < len(self.blocks) - 1:
                    pose_emb_tmp = pose_emb.repeat(1, 1, lat_h // 2, lat_w // 2, 1) # torch.Size([1, 15, 45, 45, 5120]) # torch.Size([1, 15, 52, 30, 5120]) #  # torch.Size([1, 36, 21, 104, 60])
                    # import pdb; pdb.set_trace()
