...
class Resampler(nn.Module):
    def __init__(
        ...
        self.latents = nn.Parameter(torch.randn(1, 1, dim) / dim**0.5)
        ...
        self.proj_out2 = nn.Linear(output_dim, 5120)
        self.proj_conv = nn.Conv1d(in_channels=81, out_channels=21, kernel_size=3, stride=1, padding=1)

        nn.init.zeros_(self.proj_out2.weight)
        nn.init.zeros_(self.proj_out2.bias)
    ...
    def forward(self, x, latents): # x:torch.Size([81, 1, 1024]) latents: torch.Size([81, 4, 1024])
        ...
        latents = self.proj_conv(latents).permute(1, 0, 2)
        latents =  self.proj_out2(latents)


        # latents = latents.permute(1, 0, 2)
        # import pdb; pdb.set_trace() # torch.Size([81, 1, 5120]) 
