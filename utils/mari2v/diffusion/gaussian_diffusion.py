...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        ).to(th.bfloat16)  # no noise when t == 0
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start, dtype=th.bfloat16)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = th.from_numpy(arr).to(device=timesteps.device)[timesteps].to(th.bfloat16)
    while len(res.shape) < len(broadcast_shape):
        res = res[..., None]
    return res + th.zeros(broadcast_shape, device=timesteps.device, dtype=th.bfloat16)
