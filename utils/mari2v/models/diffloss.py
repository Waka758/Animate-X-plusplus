...
class DiffLoss(nn.Module):
    ...
    def sample(self, z, temperature=1.0, cfg=1.0, device=None):
        # diffusion loss sampling
        #noise = torch.randn(z.shape[0] // 2, self.in_channels).to(device).to(torch.bfloat16)
        ...
        #     sample_fn, noise.shape, noise, clip_denoised=False, model_kwargs=model_kwargs, progress=False, device=device
        # )
