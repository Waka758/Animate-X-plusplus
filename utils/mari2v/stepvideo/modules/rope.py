...
class RoPE1D:
    def __init__(self, freq=1e4, F0=1.0, scaling_factor=1.0, spatial_len=1024):
        self.base = freq
        self.F0 = F0
    ...
    def apply_rope1d(self, tokens, pos1d, cos, sin, frame_index):
        assert pos1d.ndim == 2

        if frame_index is not None:
            if frame_index[0]:
                cos = torch.nn.functional.embedding(pos1d, cos)[:, :, None, :][:, :(frame_index[1]+1)*self.spatial_len, :, :]
                sin = torch.nn.functional.embedding(pos1d, sin)[:, :, None, :][:, :(frame_index[1]+1)*self.spatial_len, :, :]
            else:
                cos = torch.nn.functional.embedding(pos1d, cos)[:, :, None, :][:, frame_index[1]*self.spatial_len:(frame_index[1]+1)*self.spatial_len, :, :]
                sin = torch.nn.functional.embedding(pos1d, sin)[:, :, None, :][:, frame_index[1]*self.spatial_len:(frame_index[1]+1)*self.spatial_len, :, :]
        else:
...
class RoPE3D(RoPE1D):
    def __init__(self, freq=1e4, F0=1.0, scaling_factor=1.0, spatial_len=1024):
        super(RoPE3D, self).__init__(freq, F0, scaling_factor, spatial_len)
        self.position_cache = {}
    ...
    def __call__(self, tokens, rope_positions, ch_split, parallel=False, frame_index=None):
        """
        input:
            * tokens: batch_size x ntokens x nheads x dim
        ...
        for i, (D, x) in enumerate(zip(ch_split, torch.split(tokens, ch_split, dim=-1))):
            ...
            x = self.apply_rope1d(x, mesh.to(tokens.device), cos, sin, frame_index)
            out.append(x)
