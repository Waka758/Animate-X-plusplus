...
class Inference(object):
    ...
    def from_pretrained(cls, pretrained_model_path, args, device=None, **kwargs):
        ...
        if rank == 0:
            ...
            # if args.inverse-path!="":
            #     inverse_ckpt = torch.load(args.inverse-path)
            #     model.inverse.load_state_dict(inverse_ckpt)
    ...
    def __init__(
        ...
        if args.i2v_mode:
            ...
            if args.inverse_path!="":
                inverse_ckpt = torch.load(args.inverse_path)
                self.pipeline.transformer.inverse.load_state_dict(inverse_ckpt)
